...
export async function startApi() {
    ...
    // If we are not serving the UI at `/`, keep `/` as a simple health/welcome endpoint
    ...
    const normalizedPrefix = uiPrefix.trim() ? uiPrefix.trim() : '/';
    const mountRoot = normalizedPrefix === '/' || normalizedPrefix === '';
    if (!uiDir || !mountRoot) {
    ...
    if (uiDir) {
        ...
        const prefix = mountRoot ? '/' : (normalizedPrefix.endsWith('/') ? normalizedPrefix.slice(0, -1) : normalizedPrefix);
